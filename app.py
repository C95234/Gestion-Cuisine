        ...
        with cbc2:
            ...
            if bc_filled is not None:
                ...
                if suppliers_in_file:
                    ...
                    def _next_occurrence_of_weekday(target_weekday: int, from_date: dt.date) -> dt.date:
                        # 0 = lundi ... 6 = dimanche
                    ...
                    def _dates_from_jours_str(jours_raw: str, *, ref_date: dt.date):
                        ...
                        for p in parts:
                            ...
                            if idx is None:
                                continue
                            out.append(_next_occurrence_of_weekday(idx, ref_date))
                        # unique + tri
                        out = sorted(set(out))
                        return out
                    ...
                    def _suggest_slots_for_df(df_sub: pd.DataFrame):
                        ...
                        for _, r in df_sub.iterrows():
                            ...
                            if not conso_dates_row and "Jour(s)" in df_sub.columns:
                                conso_dates_row = _dates_from_jours_str(r.get("Jour(s)"), ref_date=today)
                        ...
                        if not suggested_dates:
                            ...
                            if not conso_dates_all and "Jour(s)" in df_sub.columns:
                                for v in df_sub["Jour(s)"].tolist():
                    ...
                    with dcol1:
                        d1 = st.date_input("Créneau 1 (date)", value=suggested_g1, key="global_slot_1")
                    with dcol2:
                        d2 = st.date_input("Créneau 2 (date)", value=suggested_g2, key="global_slot_2")
