...
def add_pdj_records(
    ...
    *,
    ...
    # Sécurité métier: un avoir en quantité doit toujours être négatif.
    # (évite les erreurs où l'utilisateur saisit 5 au lieu de -5).
    try:
        m = df["kind"].str.lower().eq("avoir_qty")
        if m.any():
            df.loc[m, "qty"] = -df.loc[m, "qty"].abs()
    except Exception:
        pass
