...
def export_excel(
    ...
    *,
    ...
    with pd.ExcelWriter(out_path, engine="openpyxl") as writer:
        ...
        wb = writer.book
        # --- Feuille Cuisiniers : combien produire par régime (déj/dîner) ---
        # Objectif: une feuille imprimable, utile "le matin" pour lancer les productions.
        try:
            ws_cui = wb.create_sheet("Cuisiniers")

            def _pivot_to_map(piv: pd.DataFrame) -> dict[str, dict[str, int]]:
                out = {}
                if piv is None or piv.empty or "Regime" not in piv.columns:
                    return out
                for _, r in piv.iterrows():
                    reg = str(r.get("Regime", "") or "").strip()
                    if not reg or reg.upper() == "TOTAL":
                        continue
                    out[reg] = {d: int(r.get(d, 0) or 0) for d in DAY_NAMES}
                return out

            dej_map = _pivot_to_map(dej_piv)
            din_map = _pivot_to_map(din_piv)

            thin2 = Side(style="thin", color="9E9E9E")
            border2 = Border(left=thin2, right=thin2, top=thin2, bottom=thin2)
            header_fill2 = PatternFill("solid", fgColor="EDEDED")
            total_fill2 = PatternFill("solid", fgColor="E0E0E0")
            header_font2 = Font(bold=True)
            align_left = Alignment(horizontal="left", vertical="center", wrap_text=True)
            align_center = Alignment(horizontal="center", vertical="center", wrap_text=True)

            row = 1
            ws_cui.merge_cells(start_row=row, start_column=1, end_row=row, end_column=4)
            t = ws_cui.cell(row=row, column=1)
            t.value = "FEUILLE CUISINIERS — PRODUCTIONS PAR RÉGIME"
            t.font = Font(bold=True, size=14)
            t.alignment = Alignment(horizontal="center", vertical="center")
            ws_cui.row_dimensions[row].height = 28
            row += 2

            for day in DAY_NAMES:
                ws_cui.merge_cells(start_row=row, start_column=1, end_row=row, end_column=4)
                c = ws_cui.cell(row=row, column=1)
                c.value = day.upper()
                c.font = Font(bold=True, size=12)
                c.alignment = Alignment(horizontal="left", vertical="center")
                ws_cui.row_dimensions[row].height = 20
                row += 1

                headers = ["Régime", "Déjeuner", "Dîner", "Total"]
                for col, h in enumerate(headers, start=1):
                    cc = ws_cui.cell(row=row, column=col)
                    cc.value = h
                    cc.font = header_font2
                    cc.fill = header_fill2
                    cc.border = border2
                    cc.alignment = align_center
                ws_cui.row_dimensions[row].height = 18
                row += 1

                regs = sorted(set(dej_map.keys()) | set(din_map.keys()))
                data_rows = []
                for reg in regs:
                    d1 = int(dej_map.get(reg, {}).get(day, 0))
                    d2 = int(din_map.get(reg, {}).get(day, 0))
                    tot = d1 + d2
                    if tot <= 0:
                        continue
                    data_rows.append((reg, d1, d2, tot))
                data_rows.sort(key=lambda t: (-t[3], t[0]))

                day_tot_dej = day_tot_din = day_tot = 0
                for reg, d1, d2, tot in data_rows:
                    ws_cui.cell(row=row, column=1).value = reg
                    ws_cui.cell(row=row, column=2).value = int(d1)
                    ws_cui.cell(row=row, column=3).value = int(d2)
                    ws_cui.cell(row=row, column=4).value = int(tot)

                    ws_cui.cell(row=row, column=1).alignment = align_left
                    for col in [2, 3, 4]:
                        ws_cui.cell(row=row, column=col).alignment = align_center
                    for col in [1, 2, 3, 4]:
                        ws_cui.cell(row=row, column=col).border = border2
                ...
                ws_cui.cell(row=row, column=1).value = "TOTAL"
                ws_cui.cell(row=row, column=2).value = int(day_tot_dej)
                ws_cui.cell(row=row, column=3).value = int(day_tot_din)
                ws_cui.cell(row=row, column=4).value = int(day_tot)
                for col in [1, 2, 3, 4]:
                    cc = ws_cui.cell(row=row, column=col)
                    cc.font = Font(bold=True)
                    cc.fill = total_fill2
                    cc.border = border2
                    cc.alignment = align_center if col != 1 else align_left
                row += 2

            ws_cui.freeze_panes = "A4"
            ws_cui.page_setup.fitToWidth = 1
            ws_cui.page_setup.fitToHeight = 0
            ws_cui.column_dimensions["A"].width = 34
            ws_cui.column_dimensions["B"].width = 12
            ws_cui.column_dimensions["C"].width = 12
            ws_cui.column_dimensions["D"].width = 12
        except Exception:
            pass
